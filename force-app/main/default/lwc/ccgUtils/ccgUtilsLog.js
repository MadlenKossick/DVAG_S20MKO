...
const logInfo = (
  ...
  if (logOn) {
    try {
      if (data) {
        console.log(`${source}: ${message}`, JSON.parse(JSON.stringify(data)));
      }
      else {
...
const logError = (
  ...
  try {
    if (data) {
      console.error(`${source}: ${message}`, JSON.parse(JSON.stringify(data)));
    }
    else {
...
export { logInfo, logError, reduceErrors};